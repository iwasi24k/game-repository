...
void TitleLogoPrefab::Configure(GameObject* obj) {
	...
	obj->GetTransform().scale = { SCREEN_WIDTH / 2.5f + 250.0f, SCREEN_HEIGHT / 2.5f - 200.0f, 1.0f };
	obj->GetTransform().rotation = { 1.0f, 1.0f, 0.0f };

	auto spriteComp = obj->GetComponent<SpriteComponent>();
	if (spriteComp) {
		spriteComp->LoadSprite(L"Asset\\Texture\\TitleName.png", Framework::SpriteDrawMode::Instance);
